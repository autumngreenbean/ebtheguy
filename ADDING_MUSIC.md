...
The website automatically fetches music data from `data/content.json`, which is updated by your Google Sheets. When you add albums in the sheet, they'll appear on the website automatically.
...
1. The sheet automatically updates `data/content.json` via GitHub API
2. The website reads the updated JSON file (cached for 5 minutes)
3. The album ID is dynamically loaded into the player
4. When users click the CD, tracks are fetched using the Apple Music Lookup API
5. The player displays tracks and allows playback of 30-second previews
...
4. **Refresh your website** - the new CD should appear!
